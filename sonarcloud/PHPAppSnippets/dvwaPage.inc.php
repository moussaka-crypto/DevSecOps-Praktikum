...
function dvwaSecurityLevelGet() {
    ...
    $securityLevel = isset($_COOKIE['security']) ? $_COOKIE['security'] : 'impossible';
    $validSecurityLevels = ['low', 'medium', 'high']; // whitelist
	
    if (!in_array($securityLevel, $validSecurityLevels, true)) {
        // Invalid security level, set a default (e.g., 'low')
        $securityLevel = 'low';
    }
...
function dvwaDatabaseConnect() {
	...
	if( $DBMS == 'MySQL' ) {
		...
		$db_conn = new mysqli($_DVWA['db_server'], $_DVWA['db_user'], $_DVWA['db_password'], $_DVWA['db_database'], $_DVWA['db_port']);
...
function generateSessionToken() {  # Generate a brand new (CSRF) token
	...
	$_SESSION['session_token'] = hash('sha512', bin2hex(random_bytes(64)));
}
